...
export default function FaceSetupAssistant({
      ...
      const loop = async () => {
        ...
        if (x1 !== null && y1 !== null && x2 !== null && y2 !== null && vw > 0 && vh > 0) {
          ...
          if (nw > 0.01 && nh > 0.01) {
            ...
            setAligned(ok);
            onStatus?.(ok);
            console.log("[Setup] onStatus ok:", ok); // Added for debugging
            ...
            setAligned(false);
            onStatus?.(false);
            console.log("[Setup] onStatus ok: false (no facebox)"); // Added for debugging
          }
        } else {
          setFaceBox(null);
          setAligned(false);
          onStatus?.(false);
          console.log("[Setup] onStatus ok: false (invalid coords)"); // Added for debugging
        }
  ...
  useEffect(() => {
    if (aligned && !isButtonPermanentlyEnabled) {
      setIsButtonPermanentlyEnabled(true);
      console.log("[Setup] Button permanently enabled!"); // Added for debugging
    }
    console.log("[Setup] aligned:", aligned, "isButtonPermanentlyEnabled:", isButtonPermanentlyEnabled); // Added for debugging
  }, [aligned, isButtonPermanentlyEnabled]);
  ...
  return (
    <div className="w-full h-full grid place-items-center">
      ...
      >
        ...
        {/* Bottom Start button (always present; disabled until permanently enabled) */}
        <div className="absolute inset-x-0 bottom-0 p-3 flex justify-center z-10">
          <button
            onClick={onConfirm}
