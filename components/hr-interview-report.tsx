...
export function HRInterviewReport({ onStartNewInterview }: HRInterviewReportProps) {
  ...
  const calculateStability = (yaw: number[], pitch: number[], roll: number[]): number => {
    ...
    return Math.max(0, Math.min(100, 100 - (avgMovement * 1000))) // Scale and invert
  }
  ...
  const combineProfessionalMetrics = (metrics: SessionMetrics): number => {
    ...
    return Math.round((postureScore + avgEyeContact) / 2)
  }
  ...
  const calculateConfidenceLevel = (metrics: SessionMetrics): number => {
    ...
    return Math.round((stability + eyeContact + emotionalStability) / 3)
  }
  ...
  return (
    <Card className="w-full max-w-7xl mx-auto">
      <CardHeader>
        <CardTitle className="text-3xl font-bold text-center">Enhanced Interview Analytics Report</CardTitle>
      </CardHeader>
